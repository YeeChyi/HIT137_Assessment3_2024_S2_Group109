...
# Load assets (you'll need to provide your own images)
player_image = pygame.Surface((50, 50))  # Placeholder for player image
...
class Collectible(pygame.sprite.Sprite):
    ...
    def apply_effect(self, player):
        player.health = min(player.health + 20, INITIAL_HEALTH)  # Health boost
...
def spawn_collectible():
    collectible = Collectible(SCREEN_WIDTH + random.randint(50, 300), SCREEN_HEIGHT - 70)
...
def show_level_complete_screen(level):
    screen.fill(BLACK)
    level_complete_text = font.render(f"Level {level - 1} Complete!", True, WHITE)
...
def increase_difficulty(level):
    global spawn_timer_threshold
    spawn_timer_threshold = max(30, 100 - 10 * level)  # Enemies spawn faster each level
